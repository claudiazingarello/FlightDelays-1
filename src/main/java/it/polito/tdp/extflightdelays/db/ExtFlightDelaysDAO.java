...
public class ExtFlightDelaysDAO {
	...
	public int getAirlinesNumber(Airport a) {
		String sql = "SELECT COUNT(DISTINCT (f.AIRLINE_ID)) AS tot " + 
				"FROM flights AS f " + 
	...
	public List<Rotta> getRotte(Map<Integer, Airport> idMap) {
		String sql = "SELECT  f.ORIGIN_AIRPORT_ID AS oi, f.DESTINATION_AIRPORT_ID AS di, COUNT(*) AS tot " + 
				"FROM flights AS f " + 
		...
		try {
			...
			while (rs.next()) {
				Airport sorgente = idMap.get(rs.getInt("oi"));
				Airport destinazione = idMap.get(rs.getInt("di"));
				if(sorgente != null && destinazione != null) {
					Rotta rotta = new Rotta(sorgente, destinazione, rs.getInt("tot"));
					result.add(rotta);
				} else {
